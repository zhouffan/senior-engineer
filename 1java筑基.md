...
**Java Virtual Machine**： 翻译工作，可以跨平台

- java程序(javac编译)-----java字节码-----JVM(Java类加载器->运行时数据区->执行引擎)-------操作系统函数（linux/windows/macos）
- jdk：提供了工具（exe文件等）
- jre：提供了基础类库（JDBC/ IO....）
...
- 线程私有
  ...
  - 虚拟机栈：存储当前线程运行方法所需的**数据/指令/返回地址**

    栈帧/大小限制(- Xss)

  - 本地方法栈：保存的是native方法的信息。

  - 程序计数器 ：指向当前线程正在执行的字节码指令的**地址**（作用：时间片轮转应用。记录执行到哪一行标志）
